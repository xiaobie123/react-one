...
@observer class WeatherPage extends React.Component{
    ...
    drawWeatherLine(){
        ...
        //填充高温数组
        for(let i=0;i<6;i++){
            let high = this.props.stateStore.weather.dayData[i].high;
            if(high>highest){
                high=highest;
            }else if(high<lowest){
                high=lowest;
            }
            highTem.push(Number(high.substring(3,5)))
        }
        //填充低温数组
        for(let i=0;i<6;i++){
            let low = this.props.stateStore.weather.dayData[i].low;
            if(low>highest){
                low=highest;
            }else if(low<lowest){
                low=lowest;
            }
            lowTem.push(Number(low.substring(3,5)))
        }
            ...
            <div className="weatherPage">
                ...
                <div className={'weatherBox '+ dayOrNight} style={{width:width,minHeight:height}}>
                    <header>
                        <div className="air">
                            <span>实时空气质量</span>
                            <span className="air-btn">{'pm2.5: '+this.props.stateStore.weather.pm25+' '+this.props.stateStore.weather.quality}</span>
                        </div>
                        <div className="time">
                        ...
                        this.props.stateStore.weather.dayData.map((item,index)=>{
                            return(
                                <div className="day" key={index}>
                                    <span className="perDay">{item.date.substr(-3)}</span>
                                    <i className={"iconfont "+this.switchIcon(item.type)}></i>
                                    <span className="high">{item.high.substring(3)}</span>
                                    <span className="low">{item.low.substring(3)}</span>
                                    <span className="wind">{item.fx}</span>
                                    <span className="windl">{item.fl}</span>
                                </div>
                            )
                        })  
                       }
